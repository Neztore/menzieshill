...
export const GroupDropdown: FunctionComponent<GroupDropdownProps> = function (props) {
  ...
  async function addGroup () {
    const val: string = typeof selectedValue !== "string" ? getValidGroups(groups, props.currentGroups)[0].name : selectedValue;
    ...
    if (grp) {
      ...
      newArr.push(grp);
      const success = await props.updateGroups(newArr);
      if (success) {
    ...
    <div className="field is-grouped">
      <div className="select control">
        <select value={selectedValue} onChange={(e => setSelected(e.target.value))}>
          {
                    getValidGroups(groups, props.currentGroups).map(grp => <option className="tag  " key={grp.id}>{grp.name}</option>)
                }
        </select>
      </div>
